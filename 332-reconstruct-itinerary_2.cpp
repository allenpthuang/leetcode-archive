class Solution {
 ...
 public:
  vector<string> findItinerary(vector<vector<string>>& tickets) {
    ...
    while (final_result.size() < tickets.size() + 1) {
      vector<string> result;
      worker(result, used, tickets, "JFK");
      if (result[result.size() - 1] == "JFK") result.pop_back();
      final_result.insert(final_result.begin(), result.begin(), result.end());
    }
    return final_result;
  }
};
