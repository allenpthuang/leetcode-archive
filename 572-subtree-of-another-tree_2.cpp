...
class Solution {
 private:
  ...
  bool walk_together(TreeNode* lhs_node, TreeNode* rhs_node, TreeNode*& start) {
    ...
    bool left = walk_together(lhs_node->left, rhs_node->left, start);
    if (lhs_node->val != rhs_node->val) {
      start = lhs_node;
      return false;
    }
    bool right = walk_together(lhs_node->right, rhs_node->right, start);
    return left && right;
  }
 ...
 public:
  bool isSubtree(TreeNode* root, TreeNode* subRoot) {
    ...
    while ((cur_start_node = find_node_matches(start, subRoot))) {
